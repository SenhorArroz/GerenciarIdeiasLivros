...
@section('title', $anotacao->titulo)

@section('content')
<a href="{{ route('livros.anotacoes', [$livro->slug, $livro->id]) }}" 
    ...
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="card shadow-lg">
                <div class="card-body">
                    <h1 class="fw-bold text-primary">{{ $anotacao->titulo }}</h1>
                    <p class="text-muted">Criado em {{ $anotacao->created_at->format('d/m/Y H:i') }}</p>
                    <hr>
...
<div class="position-fixed bottom-0 end-0 m-4 d-flex gap-2">
    <!-- Botão de Editar -->
    <button type="button" class="btn btn-warning btn-lg shadow-lg rounded-circle" data-bs-toggle="modal" data-bs-target="#modalEditarAnotacao">
    ...
    <!-- Botão de Deletar -->
    <form action="{{ route('anotacoes.destroy', [$livro->id, $anotacao->id]) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja excluir esta anotação?');">
        @csrf
        @method('DELETE')
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('anotacoes.update', [$livro->id, $anotacao->id]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        <label class="form-label">Título</label>
                        <input type="text" class="form-control" name="titulo" value="{{ $anotacao->titulo }}" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Descrição</label>
                        <textarea class="form-control" name="descricao" id="descricao">{!! $anotacao->descricao !!}</textarea>
                    </div>
...
<script>
    ...
    document.getElementById('modalEditarAnotacao').addEventListener('shown.bs.modal', () => {
        setTimeout(carregarEditor, 500);
    });

    document.getElementById('modalEditarAnotacao').addEventListener('hidden.bs.modal', () => {
        if (editorInstance) {
